"""Self-contained shallow modulation network approximating x^2."""
...
class ShallowModulationNetwork(nn.Module):
    r"""A shallow network that approximates :math:`x^2` with fixed modulation."""

    def __init__(self, units: int = 120, eta: float = 0.5, t: float = 1.0) -> None:
        super().__init__()
        self.units = units
        ...
        self.y = nn.Parameter(torch.zeros(units))
        self.c = nn.Parameter(torch.zeros(1))


    ##########################################################################################
    ##########################################################################################
    # try to add in the following method a condition whether modulation is true or false:
    #     ie if modulation true then modulation network will be generated else regular snn will be generated
    def forward(self, x: torch.Tensor | float, modulation: bool = True) -> torch.Tensor:
        ...
        if modulation:
            ...
            gaussian_input = torch.exp(-0.5 * (x - y) ** 2)
...
def train_and_plot(
    ...
    model = ShallowModulationNetwork(units=units)
    ...
    plt.xlabel("x")
    plt.ylabel("f(x)")
    plt.title("Shallow Modulation Network Approximating x^2")
    plt.tight_layout()
    plt.show()
