...
parser.add_argument('--stepsize', type=int, default=2000, help='Step size for the scheduler (default: 100k).')
...
plt.ylabel('Target function $f_{targ}(x)$', fontsize=16)
plt.xticks([0.0, 0.5, 1.0], fontsize=14)
plt.yticks([-1.5, -1.0, -0.5, 0.0, 0.5, 1.0], fontsize=14)
plt.tick_params(axis='both', labelsize=14)
plt.grid(True, alpha=0.5)
plt.close()
plt.savefig( f'SecondBarronFunc.pdf', bbox_inches='tight', dpi=300)
#plt.show()
...
plt.plot(x, y, label='Target Barron function', color='C0', linewidth=3)
plt.plot(x, y_pred_final, linestyle='--', label='Network approximation', color='limegreen', linewidth=2.5)
plt.xlabel('x', fontsize=16)
plt.tick_params(axis='both', labelsize=14)
...
markerline, stemlines, baseline = plt.stem(
    zero_pos[pos_in_range].detach(), weight_in_range.detach(),
    label='Weight')
plt.setp(stemlines, color='#DAA520', linewidth=2)
plt.setp(markerline, color='#DAA520', markersize=9)
plt.setp(baseline, color='C3', linewidth=2.5)
