...
dir_name = f'resultsW2pol_StepSize_{args.stepsize}_Epochs_{args.epochs}_HN_{args.units}_N_{args.size}_bp_{args.bp}_seed_{args.seed}'
...
plt.savefig(f'Target2DFunction_W2_pol.pdf', bbox_inches='tight', dpi=300)
#plt.clf()
#plt.close()
plt.show()
...
pd.DataFrame({'k': m, 'error': error_list, 'error_th': error_th}).to_csv(f'PwErrors_W2_pol.csv', index=False)
...
plt.loglog(m, error_th, color='purple', label='Theoretical error $\propto k^{-3.13}$', linewidth=2.3)
